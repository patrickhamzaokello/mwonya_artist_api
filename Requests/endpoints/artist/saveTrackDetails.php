...
if (!isset($data) || (!isset($data['trackDetails']))) {
    http_response_code(400);
    echo json_encode([
        'status' => 'error',
        'message' => 'Provide a valid trackDetails.',
    ]);
    exit;
}
...
try {
    ...
    $result = null;
    $result = $handler->insertOrUpdateTrack($data['trackDetails']);
    ...
    if ($result) {
        ...
        echo json_encode([
            'status' => 'error',
            'message' => 'trackDetails not Created.',
        ]);
    }
} catch (Exception $e) {
    http_response_code(500);
