...
if (!isset($data) || (!isset($data['mediaUploadDetails']))) {
    http_response_code(400);
    echo json_encode([
        'status' => 'error',
        'message' => 'Provide a valid mediaUploadDetails.',
    ]);
    exit;
}
...
try {
    ...
    $result = null;
    $result = $handler->saveMediaUpload($data['mediaUploadDetails']);
    ...
    if ($result) {
        ...
        echo json_encode([
            'status' => 'error',
            'message' => 'mediaUploadDetails not Created.',
        ]);
    }
} catch (Exception $e) {
    http_response_code(500);
