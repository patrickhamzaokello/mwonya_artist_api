...
if (!isset($data) || (!isset($data['releaseDetails']))) {
    http_response_code(400);
    echo json_encode([
        'status' => 'error',
        'message' => 'Provide a valid releaseDetails.',
    ]);
    exit;
}
...
try {
    ...
    $result = null;
    $result = $handler->createNewRelease($data['releaseDetails']);
    ...
    if ($result) {
        ...
        echo json_encode([
            'status' => 'error',
            'message' => 'releaseDetails not saved.',
        ]);
    }
} catch (Exception $e) {
    http_response_code(500);
