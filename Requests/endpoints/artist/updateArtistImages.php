...
if (!isset($data) || (!isset($data['artistID']) || !isset($data['imageType']) || !isset($data['uploaded_mediaID']) || !isset($data['media_creatorID']))) {
    http_response_code(400);
    echo json_encode([
        'status' => 'error',
        'message' => 'Provide a valid artistDetails .',
    ]);
    exit;
}
...
try {
    ...
    $result = null;
    $result = $handler->updateArtistProfileImage($data['artistID'], $data['imageType'], $data['uploaded_mediaID'], $data['media_creatorID']);
    ...
    if ($result) {
        ...
        echo json_encode([
            'status' => 'error',
            'message' => 'artistDetails not Created.',
        ]);
    }
} catch (Exception $e) {
    http_response_code(500);
